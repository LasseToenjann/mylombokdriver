# Informationen zur Website My Lombok Driver

Das hier ist das **Gedächtnis des Projekts**: jede Entscheidung, jeder Grund
dahinter, jede Anleitung zum Ändern und alles, was beim Bauen schiefgegangen
ist. Die `README.md` sagt nur, worum es geht — hier steht, wie und warum.

Wer in einem halben Jahr etwas ändern will, liest **Teil 1** (Anleitungen)
und schaut in **Teil 2** nach, ob es für die Sache einen Grund gab. Vieles auf
dieser Seite sieht willkürlich aus und ist es nicht.

Statische Website: HTML, CSS, Vanilla-JavaScript. **Kein Build-Schritt, keine
Abhängigkeiten.** Inhalte werden aus drei Daten-Dateien gerendert — Texte und
Preise ändert man dort, nicht im Markup.

Die Website ist **komplett auf Englisch**. Gäste kommen aus allen möglichen
Ländern — eine Sprache, die alle lesen, schlägt eine halb übersetzte Seite.
...
**Teil 1 — Etwas ändern**
* [1. Kontaktdaten eintragen](#1-kontaktdaten-eintragen)
* [2. Preise ändern](#2-preise-ändern)
* [3. Fotos austauschen](#3-fotos-austauschen)
* [4. Bewertungen pflegen](#4-bewertungen-pflegen)
* [Bewertungszahlen automatisch aktualisieren](#bewertungszahlen-automatisch-aktualisieren)
* [Foto des Fahrers einsetzen](#foto-des-fahrers-einsetzen)
* [Das Logo](#das-logo)

**Teil 2 — Warum es so aussieht, wie es aussieht**
* [Bewegung und Animationen](#bewegung-und-animationen)
* [Zwei Seiten statt einer](#zwei-seiten-statt-einer)
* [Find your match: die vier Fragen](#find-your-match-die-vier-fragen)
* [Der Bilderstand](#der-bilderstand)

**Teil 3 — Technik und Betrieb**
* [Struktur](#struktur)
* [Funktionen](#funktionen)
* [Lokal starten](#lokal-starten)
* [Veröffentlichen (GitHub Pages)](#veröffentlichen-github-pages-kostenlos)
* [SEO](#seo)
* [Search Console: was zu tun ist](#search-console-was-zu-tun-ist)

**Teil 4 — Geprüft**
* [Was durchgetestet ist](#was-durchgetestet-ist)
* [For the owner (English)](#for-the-owner-english)

---

## Vor dem Livegang: zwingend, erledigt
...
### Wenn „die Animationen gehen nicht" gemeldet wird

Das ist genau einmal passiert, und die Ursache lag nicht im Code. **Der
Energiesparmodus von Android schaltet Systemanimationen ab und meldet dem
Browser `prefers-reduced-motion: reduce`** — die Seite macht dann bewusst alles
sofort sichtbar, ganz ohne Einblenden. Auf dem iPhone macht die Einstellung
„Bewegung reduzieren" dasselbe. Beides springt bei niedrigem Akkustand von
selbst an, deshalb wirkt es sporadisch.

Bevor irgendjemand am Reveal-System schraubt, in dieser Reihenfolge prüfen:

1. **Energiesparmodus aus, Seite neu laden.** Erledigt den Fall fast immer.
2. **Fehlerbild unterscheiden.** Karten bleiben *leer* → der Beobachter hängt,
   das wäre ein echter Fehler. Karten sind *sofort da*, nur ohne Einblenden →
   reduzierte Bewegung oder ein alter Stand im Browser-Cache.
3. **Hart neu laden**, falls der Browser noch eine ältere `app.js` hält.

Nachgestellt und geprüft wurden: langsames Scrollen, Sprung ans Seitenende,
Ankerklick aus dem Hero, Direkteinstieg über `#tours`, Filterwechsel, Neuladen
mit wiederhergestellter Scrollposition und die Zurück-Taste. In allen Fällen
kommen alle Elemente an.
...
## Find your match: die vier Fragen
...
## Was durchgetestet ist

Zum Abschluss des Baus wurde die ganze Seite noch einmal von vorn geprüft —
Texte, Daten, Verhalten, Layout. Was dabei herauskam, ist behoben; hier steht,
womit geprüft wurde, damit man es nach einer Änderung wiederholen kann.

**Automatisch nachstellbar** (headless Chromium, Playwright):

| Geprüft | Ergebnis |
|---|---|
| Statischer Text im Markup vs. Stringtabelle | deckungsgleich, sonst sieht ein Crawler ohne JavaScript etwas anderes |
| Horizontaler Überlauf bei 360 / 390 / 768 / 1280 / 1920 px | keiner |
| Touchflächen auf Pixel 7 und iPhone 13 | alles über 40 px außer dem Logo (38 px, kein Bedienelement) |
| Tote Sprungmarken | keine |
| Tote CSS-Klassen | keine |
...
**Was dabei gefunden und behoben wurde** — als Beispiel dafür, wonach man
suchen muss:

* Die Matcher-Überschrift stand im Markup anders als in der Stringtabelle.
* Preise erschienen in zwei Schreibweisen: die Karten mit Punkten
  (`IDR 350.000`), jeder von Hand geschriebene Preis in Hinweisen und FAQ mit
  Kommas. Der Code folgt jetzt der Prosa und schreibt `IDR 350,000`.
* Die FAQ versprach „you will not be asked for money at a gate", während die
  Gili-Schnorcheltour die Inselgebühr ausnimmt.
* Die Sembalun-Tour war nach ihrer Erweiterung noch als `pace: 'medium'`
  hinterlegt — zwölf Stunden mit Fußweg sind kein mittlerer Tag.
* Der Fußzeile fehlte „About".

**Zwei Beobachtungen ohne Codeänderung**, weil sie dem Betreiber gehören:

1. **Wasserfall-Tour und Sembalun-Tour überschneiden sich.** Seit Sendang Gile
   und Tiu Kelep auch im Sembalun-Tag stecken, enthält die 1.450.000er Tour
   alles, was die 1.300.000er enthält, plus das Sembalun-Tal. Nebeneinander auf
   der Startseite sieht die kürzere Tour dadurch schwach aus. Im Hinweistext
   der langen Tour steht deshalb ein Satz, dass die Wasserfälle allein der
   kürzere und leichtere Tag sind.
2. **Fünf der dreizehn Touren können im Matcher nie gewinnen.** Bei vier Fragen
   und dreizehn Touren ist das unvermeidlich; sie erscheinen aber unter „Also
   worth a look". Wer das ändern will, braucht eine fünfte Frage oder feinere
   `match`-Felder — nicht mehr Gewichtung.
...
- **`js/content.js`** — your tours, your prices, the FAQ

To change a price, find your tour in `js/content.js` and edit the `price:` line.
It is in Rupiah, written without separators: `800000` shows as `IDR 800,000`.

To use your own photos, put them in the `images/` folder and change the `scene:`
line of that tour to the file name: `scene: 'your-photo.webp'`.

**The reviews on the site are real Google reviews**, copied from your profile.
The previous driver's name was replaced with "our driver" throughout; nothing
else about them was changed. The rating and the number under the header come
from `js/config.js` and are typed in by hand — check them against your Google
profile now and then.

**No animal is touched in any photo on this site.** One stock picture showed a
guide holding a pufferfish for a child; it was removed at your request. Please
keep that rule when new photos are added.
